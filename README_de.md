...
Persönliche Baseline für kleine Linux-Hilfsscripts. Das Projekt stellt ein Installer-Script bereit, mit dem weitere Scripts nach `$HOME/.local/bin` oder in ein frei wählbares Zielverzeichnis installiert werden können.
...
- `maintenance-upgrade`: führt `apt update`, `apt upgrade -y` und `apt autoremove -y` aus.
- `bcache-monitor`: installiert das Linux-Bcache-Monitor-Script.
